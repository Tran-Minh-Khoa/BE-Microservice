...
function setupRedisConnection(redis) {
    ...
    redis.on("message", async (channel, message) => {
        if (channel === "real-time-quiz") {
            try {
                const gameResultData = JSON.parse(message);
                console.log(gameResultData);
                const gameData = await fetchGameData(gameResultData.gameId);

                //const now = new Date();
                //const expiryDate = new Date(now.setDate(now.getDate() + 30));

                for (const user of gameResultData.users) {
                    const newVoucher = {
                        user_id: user.id,
                        qr: tempQR,
                        status: "Active",
                        voucher_template_id: gameData.voucher_template_id,
                        brand_id: gameData.brand_id,
...
async function fetchGameData(gameId) {
    try {
        const response = await axios.get(`http://api-gateway:8000/game/${gameId}`, {
            headers: {
